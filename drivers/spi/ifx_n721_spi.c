...
static irqreturn_t ifx_spi_handle_srdy_irq(int irq, void *handle)
{
	...
	if(pin_val == 0)
	{
		IFX_SPI_DEBUG("[SPI][SRDY_IRQ] pin value is ZERO.. Return!! \n");
...
static void ifx_spi_handle_work(struct work_struct *work)
{
	...
	{
		pm_off_count = 1;
		IFX_SPI_DEBUG("[SPI][handle_work] ifx_spi_handle_work INFO spi_data->is_suspended is (0x%x)\n", spi_data->is_suspended);
		...
		IFX_SPI_DEBUG("[EBS] ifx_spi_handle_work INFO EXIT is_suspend = 0x%x pm_off_count=%d\n", spi_data->is_suspended, pm_off_count);

		if(1 == spi_data->is_suspended)
		{
			// To Do how to handle the PM OFF state during 1sec
			IFX_SPI_DEBUG("[SPI][handle_work] ifx_spi_handle_work error is_suspended is (0x%x)\n",spi_data->is_suspended);
		}
	}
	// 20120904 jisil.park
