...
static void lp8720_enable(struct i2c_client* client, int status)
{
	...
	printk(KERN_DEBUG "  lp8720_enable: %d\n", status);
	...
	if (status == 0) {	// Standby mode
		...
		if (enabled == 0) {
			i2c_smbus_write_byte_data(client, LP8720_LDO1_SETTING, 0x0c);	// 1.8V
			i2c_smbus_write_byte_data(client, LP8720_LDO2_SETTING, 0x19);	// 2.8V
			i2c_smbus_write_byte_data(client, LP8720_LDO3_SETTING, 0x19);	// 2.8V
			i2c_smbus_write_byte_data(client, LP8720_LDO4_SETTING, 0x11);	// 1.8V
			i2c_smbus_write_byte_data(client, LP8720_LDO5_SETTING, 0x19);	// 2.8V
			i2c_smbus_write_byte_data(client, LP8720_BUCK_SETTING1, 0x09);	// Buck = 1.2V
			i2c_smbus_write_byte_data(client, LP8720_BUCK_SETTING2, 0x09);	// Buck = 1.2V

			data = i2c_smbus_read_byte_data(client, LP8720_LDO1_SETTING);
			printk(KERN_DEBUG "[lp8720_enable] LP8720_LDO1_SETTING: %d\n", data);
			data = i2c_smbus_read_byte_data(client, LP8720_LDO2_SETTING);
			printk(KERN_DEBUG "[lp8720_enable] LP8720_LDO2_SETTING: %d\n", data);
			data = i2c_smbus_read_byte_data(client, LP8720_LDO3_SETTING);
			printk(KERN_DEBUG "[lp8720_enable] LP8720_LDO3_SETTING: %d\n", data);
			data = i2c_smbus_read_byte_data(client, LP8720_LDO4_SETTING);
			printk(KERN_DEBUG "[lp8720_enable] LP8720_LDO4_SETTING: %d\n", data);
			data = i2c_smbus_read_byte_data(client, LP8720_LDO5_SETTING);
			printk(KERN_DEBUG "[lp8720_enable] LP8720_LDO5_SETTING: %d\n", data);			
		}
	}
...
static DEVICE_ATTR(cam, 0666, lp8720_show_cam, lp8720_store_cam);
static DEVICE_ATTR(af, 0666, lp8720_show_af, lp8720_store_af);
static DEVICE_ATTR(vt, 0666, lp8720_show_vt, lp8720_store_vt);
...
static int lp8720_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = regulator_set_voltage(regulator_vddio_vi, 1800000, 1800000);
	if (err) {
	...
	printk(KERN_DEBUG "    : lp8720 enable gpio num: %d)\n", pdata->en_gpio_num);
