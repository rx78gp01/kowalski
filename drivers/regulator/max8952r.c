/*
 * max8952r.c -- support regulators in max8952
 *
...
static const int max8952_mode1_voltages[] = {
        ...
        670000, 710000, 700000,  750000,  780000,  830000,  870000,  900000,  930000,  980000,
...
{
	const struct max8952_info *reg = NULL;
	unsigned int dev_id = 0;

	dev_id = rdev_get_id(rdev);
	if (dev_id < MAX8952_MODE0 || dev_id > MAX8952_MODE3)
		return -EINVAL;

	reg = &max8952_regulators[dev_id];

	return reg->voltages_list[selector];
}
