...
static void battery_update(struct battery_info *batt_info)
{
	...
	/* AT Boot State */
	if ((at_boot_state == 1) && (batt_info->present == 0)) 
		batt_info->present = 1;
	...
	if (( at_boot_state == 1 ) || ( at_charge_index == 1 ) || ( ELT_test_mode == 1 ))
	{
...
{
	DBG("[BATTERY SHUTDOWN] notification_of_changes_to_battery\n");
	if(refer_batt_info == NULL)
		return;
...
static int __init array_tp_boot_state(char *str)
{
	...
	printk(KERN_INFO "array_tp_boot_state = %d\n", array_tp_boot);
